...
class Video:
  ...
  # returns True if we tried to load the video, whether it failed or not
  def loaded(self):
    return self.json != None

  # returns True if self holds data for a valid video
  def valid(self):
    return self.loaded() and self.json != {}
  ...
  def get_source(self, quality='1080p'):
    if not self.valid():
      logger.debug("[video] tried to get a source on an unloaded or invalid video")
      return None
  ...
  def get_best_quality(self):
    if not self.valid():
      logger.debug("[video] tried to get the best quality on an unloaded or invalid video")
      return None
